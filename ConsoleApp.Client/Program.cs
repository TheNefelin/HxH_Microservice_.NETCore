...
string server = "https://localhost:7108";
using var channel = GrpcChannel.ForAddress(server);
var client = new HunterNenProto.HunterNenProtoClient(channel);

var getAll = await client.HunterNenGetAllAsync(new Empty());
foreach (var h in getAll.HunterNens)
{
    Console.WriteLine($"- {h.IdHunter}, {h.IdNen}, {h.NenLevel}");
}
